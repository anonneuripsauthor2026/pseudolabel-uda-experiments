...
class KGLM_covariate_shift:
    ...
    def fit(self, rho = 0.5, beta = 2, lbd_min = None, lbd_max = None, lbd_tilde = None, n_lambda = None): # KRR under covariate shift
        assert beta > 1
        ...
        lbd_min_val = lbd_min if lbd_min is not None else 0.1 / self.n
        lbd_tilde_val = lbd_tilde if lbd_tilde is not None else lbd_min_val  # imputation model; default: lbd_min (undersmoothed)
        lbd_max_val = lbd_max if lbd_max is not None else 1.0
        if n_lambda is not None:
            # use n_lambda points with geometric spacing
            m = int(n_lambda)
            m = max(m, 2)
            self.Lambda = lbd_min_val * ((lbd_max_val / lbd_min_val) ** (np.arange(m) / (m - 1)))
        else:
            # original logic: beta determines step ratio
            m = np.log(lbd_max_val / lbd_min_val) / np.log(beta)
            m = max( int(np.ceil(m)) , 2 ) + 1
            self.Lambda = lbd_min_val * ( beta ** np.array( range(m) ) ) # for training

        # pseudo-labeling (call the KRR solver in sklearn)
        mdl = RKHSGLM_Scaled(family=self.family, kernel=self.kernel, lam=lbd_tilde_val, fit_intercept=False)# clip_f=15, cg_tol=1e-7, precond="jacobi")
